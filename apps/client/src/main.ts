...
const myTurnText = (steps: number): string =>
  `🎲 <b>내 턴</b> · 남은 이동 ${steps}칸 · 방에서 [추리하기]`;
...
// 같은 턴에 [신고하기]/[내 턴 종료]를 고를 창을 열어 둔다. 클라에 핸들러가 없어 이 안내가
// 화면에 아예 없었다.
//
...
//     `currentTurn`이 바뀜. 사용자가 [신고하기]/[내 턴 종료]를 실제로 보낸 순간도 정리한다.
//
...
const accuseSubText = (): string => {
  ...
  return `⏳ 지금 신고할 수 있어요 (${left}초). 넘기려면 [내 턴 종료]`;
};
...
// 07-30 라벨 교체(추리하기·신고하기·지름길로 가기·엿보기·내 턴 종료) 후 **동사를 라벨에 맞춘다**
// (ui-copy §8.1 「같은 것을 두 이름으로 배우게 하지 않는다」). 아래 둘은 버튼 3종이 공유하므로
// **행위 중립**으로 둔다 — 예전 문안은 `고발할`이어서 [지름길로 가기]·[엿보기]·[내 턴 종료]가
// 자기 것이 아닌 동사를 말하고 있었다.
const NOT_STARTED = "잔치가 시작되면 쓸 수 있어요.";
const ELIMINATED = "탈락한 뒤에는 쓸 수 없어요. 반증만 이어집니다.";
...
const updateActionButtons = (state: Room["state"]): void => {
  ...
  // 추리하기 (§5.1) — 판정 순서를 지킬 것(탈락 + 내 턴 아님이 겹칠 때 탈락이 먼저)
  setAction(
    "suggest",
    idle
      ? "잔치가 시작되면 추리할 수 있어요."
      : elim
        ? "탈락한 뒤에는 추리할 수 없어요. 반증만 이어집니다."
        : !mine
          ? NOT_MY_TURN
          : !me?.room
            ? "방 안에서만 추리할 수 있어요. 입구(🚪)로 들어가세요."
            : null,
    "이 방에서 용의자와 훔친 것을 지목해요. 장소는 현재 방으로 고정.",
  );

  // 신고하기 (§5.2) — 창이 열려 있으면 활성 툴팁을 §5.2 "추리 직후" 문안으로.
  // 이 버튼만 «신고» 동사를 쓴다(공용 문안은 행위 중립이라 여기서 덮어쓴다).
  setAction(
    "accuse",
    idle
      ? "잔치가 시작되면 신고할 수 있어요."
      : elim
        ? "이미 신고에 실패했어요. 남은 역할은 반증입니다."
        : !mine
          ? NOT_MY_TURN
          : null,
    accuseDeadline !== null
      ? "지금 신고할 수 있어요. 넘기려면 [내 턴 종료]."
      ...
      : elim
        ...
        : !mine
          ...
          : !me?.room
            ? "지름길은 방 안에서만 쓸 수 있어요."
            : !dest
              ? "이 방에는 지름길이 없어요. (지름길은 3쌍)"
              : null,
    me?.room && dest
      ? `지름길 — ${label(me.room)} → ${label(dest)}. 주사위 없이 즉시.`
      : "",
  );
    ...
    idle
      ...
      : elim
        ...
        : !mine
          ? NOT_MY_TURN
          : nearAny && !nearUnused
            ? "이 NPC는 이미 엿봤어요. 다른 NPC를 찾아보세요."
            : !nearUnused
              ? "엿볼 수 있는 이가 근처에 없어요. 보드 가장자리의 NPC 곁으로."
              : null,
    "다가가기 — 근처 NPC에게 다가가 이동 보너스. NPC마다 1회.",
  );
...
// 방에 못 들어가고 → `[추리하기]`이 영구 비활성 → 제출물 ①의 "웹에서 즉시 플레이"가 절반만
// 성립한다.
//
