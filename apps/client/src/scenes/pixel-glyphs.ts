...
export const ZODIAC_BADGE: Record<string, DotGrid> = {
  ...
  // 돼지 — 통통한 아랫배.
  pig: ["..#", "..#", "###"],
  // 팀 카메오 계략 NPC — 쿼카(귀 둘 + 앉은 몸), 안드로(H형 로봇 실루엣).
  quokka: ["#.#", ".#.", "###"],
  android: ["#.#", "###", "#.#"],
};
...
/**
 * 소환 앵커 마크 3×3(spec §5 행 18) — 종(鐘) 실루엣.
 *
 * 뷰1·2·3이 쓰는 `SUMMON_ANCHOR_ICON`(🔔)과 **같은 정보**를 도트 문법으로 옮긴 것이다.
 * 뷰4는 이모지를 쓸 수 없고(안티에일리어싱·비도트 폰트), 그렇다고 마크를 빼면
 * "이 칸이 소환 자리"라는 정보가 뷰4에서만 사라진다.
 *
 * 배지 12종·스탬프 6종과는 **다른 오브젝트**(바닥 칸)에 붙으므로 사전 간 거리는 제약이
 * 아니지만, 두 사전 어느 항목과도 완전히 일치하지 않게 잡았다(같은 3×3이 두 뜻으로
 * 읽히는 것을 막는다).
 */
export const SUMMON_MARK: DotGrid = [".#.", "###", "###"];
