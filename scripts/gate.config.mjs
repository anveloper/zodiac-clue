...
export const SCREEN = {
  ...
  rightColumn: {
    ...
    knownUnmeasured:
      ...
      "판 중반(이 화면)뿐이고 여기서는 S8만 돈다 — 즉 이 표는 계측 사각지대에 있다. " +
      "실제로 재 보면 결함이 있다: `.sg-n`(12px `#6F5D49`)이 일반 행(`.sg-row` = `--wds-tint-primary` " +
      "`#F3E1D6`) 위에서 **4.96:1**로 아슬하게 통과하지만, **내 제안 행(`.sg-mine`, " +
      "`rgba(180,98,43,0.14)`)에서는 캔버스 색에 따라 **4.10~4.49:1로 AA(4.5) 미달**이다(재실측 — " +
      "검정 4.10 · `#1f2b3a` 4.24 · 보드 초록 4.49 · 밝은 판 4.96). " +
      "『2026-08-26 정정 — 여기 있던 «`.sg-n` 10px `#8f8574` on `#1f2b3a` = 3.94:1» 은 다크 테마 시절 " +
      "실측이고 플랜 21(a11y)이 색을 이미 바꿨다. 함께 있던 «촬영(08-01) 전 동결 구간이라 재기만 한다»도 " +
      "유효기간이 지난 사유다. 다만 **결함 자체는 색만 바뀐 채 남아 있었다** — 값을 갱신해 다시 적는다.』",
