...
export const SCREEN = {
  ...
  rightColumn: {
     ...
     * CSS의 `min-height: 72px` 하나뿐이고, 거기서 머리글·패딩을 빼면 본문에 남는 것은
     * **18.7px = 0줄**이다(실측). 즉 «패널이 자기 최소 몫을 확보하는가»의 답이 «아니오»다.
     * 위 패널 합이 컬럼을 넘는 순간(증거 노트 260px 고정 + 제안 기록표 32% 상한 +
     * AI 대사 패널이 폴백 사유만큼 자람) 로그는 그 바닥으로 떨어지고 아무것도 전달하지 못한다.
     *
     ...
     */
    knownUnmeasured:
      "제안 기록표가 떠 있는 **데스크톱 결과 화면**을 S6으로 재면 `.sg-n`(제안 순번)이 " +
      "10px `#8f8574` on `#1f2b3a` = **3.94:1**로 WCAG AA(4.5) 미달이다(실측). " +
      "진짜 결함이지만 고치려면 CSS를 건드려야 하고, 지금은 촬영(08-01) 전 화면 동결 구간이라 " +
      "«재기만 한다»가 이번 작업의 범위다. 그래서 S8은 결과 화면 계측이 **끝난 뒤** 3판에서 돌아 " +
      "그 화면을 오염시키지 않는다 — 대신 이 문장이 매 실행 남는다. 촬영 뒤 고칠 목록의 첫 줄.",
