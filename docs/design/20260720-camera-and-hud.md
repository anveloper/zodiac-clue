...
<!-- 갱신 2026-07-22: 바이너리 2D↔2.5D 토글 → 순서형 3단계 "뷰 진화 단계 선택기" -->
- 좌하단 **[▶ 뷰N]** 버튼으로 **순서형 단계 전환**: 뷰1 `2d-emoji`(Phaser 탑다운) → 뷰2 `three-emoji`(Three.js 빌보드) → 뷰3 `three-asset`(Three.js + 에셋 아트) → (순환). 버튼은 "다음 갈 단계"를 표기. 새 단계는 `STAGES` 배열에 push만 하면 편입(뷰4+ 3D 등). 서버·HUD·입력 규칙 무변경(렌더러만 교체). ~~좌하단 [2.5D] 버튼으로 2D↔2.5D 즉시 전환~~
- 서버 상태가 정수 그리드라 렌더 독립 → three는 `(gx,gy)→(x,0,z)` 매핑만. 두 렌더러 공존, **활성 뷰만 입력 처리**. **`#game`(Phaser)은 절대 숨기지 않고** three 캔버스를 위에 얹어 가리기만(z-index: Phaser 0 < 캔버스 2 < 버블 3 < HUD 5) → 뷰1 복귀 시 빈 화면이 되던 버그 방지.
