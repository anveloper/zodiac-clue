...
- **NPC 추리(decideTurn)**: 각 봇의 후보집합(용의자/장물/장소)에서 자기 손패·자기 제안의 반증·**공유 공개카드(revealed)**를 소거 → 남은 후보로 제안, 확신 시에만 고발.
  - 반증 안 됨 + 자기 미보유 → 그 3장이 봉투(정답) → **정확한 고발**. 봇은 틀린 고발을 하지 않음.
  - 후보가 각 1개로 좁혀지면 고발.
- LLM 장애와 무관하게 게임 진행·판정은 항상 정상.

### 2) 표현 계층 — LLM (Gemini, `ai/narrator.ts`)
- 입력: 규칙엔진이 확정한 **결정된 정보만**(행동·용의자·장물·장소·성격·말투).
- 출력: 사극풍 대사 **한 문장**. systemInstruction으로 형식·길이·진실값 생성 금지 계약.
- 행동 종류: `suggest`(제안) · `accuse`(고발) · **`scheme`(계략 귓속말)**.
- **성격(PERSONA)·말투(VOICE tone)** 주입 → 같은 결정도 캐릭터별로 말투가 달라짐.
- **폴백(fallbackLine)**: 키 없음/타임아웃/429/오류 시 규칙기반 사극 대사. intro/outro로 캐릭터색 유지.
- **계략 귓속말(scheme)**: 고정 NPC가 흘리는 대사 — **당사자에게만 전문**(엿본 단서 포함) private 전송, 타인에겐 `(귓속말)`만 브로드캐스트. 정보는 규칙엔진이 확정(엿본 카드), LLM은 표현만.
...
## 사람다운 템포
- NPC 행동 딜레이 = **사용자 평균 턴시간의 절반**(EMA 측정, 1800~7000ms 클램프).
- 턴 내 **2박자**(방 이동 → 잠깐 쉬고 → 제안)로 사용자가 흐름을 인지.

## 검증 예시 (결정 vs 표현 분리)
- 동일 **결정**: 제안 = 뱀 무녀 / 술동이 / 후원
  - 잔나비 광대(익살): "낄낄, 후원서 술동이를 훔쳤다니 아주 볼만하구먼!"
  - 닭 훈장(훈계조): "쯧쯧, 술동이를 훔치다니 엄히 다스림이 마땅하렷다."
...
## 주요 프롬프트·지시 (실제)
- **systemInstruction**: "너는 조선 사극풍 추리 보드게임 NPC다. 배경: 호랑이 대감의 생신 잔치에서 누군가 잔치 음식·선물을 훔쳤다. 누가(도둑)·무엇을(훔친 것)·어디서(장소)를 추리한다. … 오직 대사 한 문장만 출력 … 게임의 정답이나 남의 손패를 아는 척 금지, 주어진 정보만 사용."
...
- **Phaser 3**(MIT) 2D 렌더 · **Three.js**(MIT) 2.5D 렌더(뷰2/뷰3) · **Vite**(MIT) 빌드 · **TypeScript**(Apache-2.0)
- **Google Gemini API**(`gemini-flash-lite-latest`) — NPC 대사 생성
- **그래픽/아이콘**: 보드·토큰은 코드(Phaser Graphics / Three 지오메트리) 생성이 기본, 아이콘은 **시스템 유니코드 이모지**. <!-- 갱신 2026-07-22 --> 뷰3(에셋)용 **placeholder 이미지 44종**(`scripts/gen-placeholder-assets.mjs` → `public/assets/`) 추가, 추후 GPT 이미지 2.0 PNG로 교체. 폰트는 시스템 폰트.
