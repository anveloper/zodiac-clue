...
- `hold/`   : 보류 작업(방향 미확정·우선순위 밀림). 재개 시 `git mv hold/<x>.md active/`.
- `done/`   : 완료 작업. 완료 시 `git mv active/<x>.md done/`.
- **취소·문맥 소멸**은 `done/`이 아니다 → `docs/archive/plans/`로 `git mv`하고 **첫 제목 줄 뒤에 상태 배너**를 넣는다
  («⛔ 취소 (날짜) — 사유 · 근거 문서»). `done/`에 넣으면 "완료했다"는 거짓 기록이 남는다.

## 규칙
- 작업 시작 → `active/`에 플랜 생성. 완료 → `done/`으로 이동. 보류 → `hold/`로 이동(상태·사유 명시).
